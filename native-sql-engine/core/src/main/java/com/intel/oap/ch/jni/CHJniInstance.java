 ...
 * libasyncProfiler.so.
 */
public class CHJniInstance {

    private static CHJniInstance instance;

    private String currLibPath = "";

    private CHJniInstance() {
    }

    public static CHJniInstance getInstance() {
        return getInstance(null);
    }

    public static synchronized CHJniInstance getInstance(String libPath) {
        if (instance != null) {
            return instance;
        }
        ...
        if (!libPathExists) {
            ...
            try {
                InputStream is = CHJniInstance.class.getResourceAsStream(soFileName);
